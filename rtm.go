...
func (api *Client) StartRTMContext(ctx context.Context) (info *Info, websocketURL string, err error) {
	...
	err = postForm(ctx, api.httpclient, SLACK_API+"rtm.start", url.Values{"token": {api.token}}, response, api.debug)
	if err != nil {
...
func (api *Client) ConnectRTMContext(ctx context.Context) (info *Info, websocketURL string, err error) {
	response := &infoResponseFull{}
	err = postForm(ctx, api.httpclient, SLACK_API+"rtm.connect", url.Values{"token": {api.token}}, response, api.debug)
	if err != nil {
...
// RTMOptionWebsocketDialer set the dialer to be used when making websocket connections.
func RTMOptionWebsocketDialer(d *net.Dialer) RTMOption {
	return func(rtm *RTM) {
		rtm.dialer = d
	}
}
...
// NewRTMWithOptions Deprecated just use NewRTM(RTMOptionUseStart(true))
