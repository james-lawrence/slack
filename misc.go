...
func fileUploadReq(ctx context.Context, endpoint, fieldname, filename string, values url.Values, r io.Reader) (*http.Request, error) {
	var (
		err      error
		req      *http.Request
		iowriter io.Writer
...
func postWithMultipartResponse(ctx context.Context, client HTTPRequester, endpoint, name, fieldname string, values url.Values, r io.Reader, intf interface{}, debug bool) error {
	req, err := fileUploadReq(ctx, endpoint, fieldname, name, values, r)
	if err != nil {
		return err
	}
